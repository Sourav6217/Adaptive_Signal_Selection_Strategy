...
#  PLOTLY TEMPLATE
# ─────────────────────────────────────────────
# Shared dark layout kwargs — passed directly to fig.update_layout()
DARK_LAYOUT = dict(
...
def load_data():
    ...
    # ── Metrics helper — explicit ret→cum mapping ──
    ret_to_cum = {
        "returns":             "cum_market",
        "strategy_returns":    "cum_strategy",
        "mr_strategy_returns": "cum_mr",
        "ml_strategy_returns": "cum_ml",
        "bandit_returns":      "cum_bandit",
        "bandit_imp_returns":  "cum_bandit_imp",
    }

    def metrics(ret_col):
        r = df[ret_col].dropna()
        sharpe = np.sqrt(252) * r.mean() / r.std() if r.std() > 0 else 0
        cum_col = ret_to_cum[ret_col]
        cum = df[cum_col]
        total = float(cum.iloc[-1])
        roll_max = cum.cummax()
        mdd = float(((cum - roll_max) / roll_max).min()) * 100
        wins = float((r > 0).sum() / len(r) * 100)
        return dict(sharpe=round(sharpe, 3), total=round(total, 3),
                    mdd=round(mdd, 1), win=round(wins, 1))

    perf = {
        "Market":   metrics("returns"),
        "Momentum": metrics("strategy_returns"),
        "Mean Rev": metrics("mr_strategy_returns"),
        "ML":       metrics("ml_strategy_returns"),
