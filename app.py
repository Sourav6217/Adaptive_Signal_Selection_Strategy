...
/* Hide the default Streamlit sidebar collapse/expand arrow entirely */
button[data-testid="collapsedControl"],
button[kind="header"] { display: none !important; }

/* ── App background ── */
.main, .stApp { background: #0f172a !important; }
...
/* ── Section titles — SOLID COLOR (no webkit gradient, visible everywhere) ── */
.section-title {
    font-size: 2rem;
    font-weight: 700;
...
.section-sub { color: #94a3b8 !important; font-size: .95rem; margin-bottom: 2rem; }
...
/* ── Hero title — SOLID fallback so it's never invisible ── */
.hero-title {
    font-size: 2.6rem;
    font-weight: 800;
    color: #60a5fa !important;
...
.hero-sub { font-size:1.05rem; color:#94a3b8 !important; margin-top:.5rem; line-height:1.6; }

/* ── All headings — force visible dark-theme color ── */
h1, h2, h3, h4, h5 { color: #e2e8f0 !important; -webkit-text-fill-color: #e2e8f0 !important; }
...
/* ── Sidebar toggle button (custom, always visible) ── */
#sidebar-toggle-btn {
    position: fixed;
    top: 14px;
    left: 14px;
...
#  SIDEBAR
# ─────────────────────────────────────────────
# Inject a persistent sidebar open button (always visible even when sidebar is collapsed)
st.markdown("""
<script>
function toggleSidebar() {
    ...
    if (btn) { btn.click(); }
        ...
        // Try the expanded close button
        const closeBtn = window.parent.document.querySelector('button[kind="header"]');
        if (closeBtn) closeBtn.click();
    }
}
</script>
<button id="sidebar-toggle-btn" onclick="toggleSidebar()" title="Toggle Navigation">☰</button>
""", unsafe_allow_html=True)
...
def section_header(icon, title, subtitle):
    st.markdown(
        ...
        f"<div style='font-size:2rem;font-weight:700;color:#60a5fa;"
        f"-webkit-text-fill-color:#60a5fa;margin-bottom:.3rem;'>{title}</div>"
        f"<div style='color:#94a3b8;font-size:.95rem;margin-bottom:2rem;'>{subtitle}</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
if page == sections[0]:
    ...
    st.markdown(
        ...
        "<div style='font-size:2.6rem;font-weight:800;color:#60a5fa;-webkit-text-fill-color:#60a5fa;line-height:1.2;'>Adaptive Signal Selection</div>"
        "<div style='font-size:1.9rem;font-weight:800;color:#818cf8;-webkit-text-fill-color:#818cf8;line-height:1.2;'>under Market Regime Uncertainty</div>"
        "<div style='font-size:1.05rem;color:#94a3b8;margin-top:.5rem;line-height:1.6;' style='max-width:720px;margin-top:1rem;'>"
    ...
    with col_r:
        st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>Strategy Scoreboard</h3>", unsafe_allow_html=True)
        rows_sb = [
...
# ═══════════════════════════════════════════════════════════
    ...
    with col_b:
        st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>Signal Logic</h3>", unsafe_allow_html=True)
        st.markdown(
            "<div class='formula'>"
...
# ═══════════════════════════════════════════════════════════
    ...
    st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>Risk-Adjusted Metrics</h3>", unsafe_allow_html=True)
    rows = []
...
# ═══════════════════════════════════════════════════════════
    ...
    col_l, col_r = st.columns([3, 2])
    with col_l:
        st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>UCB Algorithm</h3>", unsafe_allow_html=True)
        st.markdown(
            "<div class='formula'>"
    ...
    with col_r:
        st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>Performance Metrics</h3>", unsafe_allow_html=True)
        for name, m, clr in [
    ...
    st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>Why the Bandit Underperforms</h3>", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with c1:
            st.markdown("<h4 style='color:#cbd5e1 !important;-webkit-text-fill-color:#cbd5e1 !important;font-size:1.05rem;font-weight:600;margin-bottom:.6rem;'>Signal 1: Momentum</h4>", unsafe_allow_html=True)
            st.markdown(
                "<div class='formula'>"
                "ma_short = Close.rolling(10).mean()<br>"
                "ma_long  = Close.rolling(30).mean()<br><br>"
        ...
        with c2:
            st.markdown("<h4 style='color:#cbd5e1 !important;-webkit-text-fill-color:#cbd5e1 !important;font-size:1.05rem;font-weight:600;margin-bottom:.6rem;'>Signal 2: Mean Reversion</h4>", unsafe_allow_html=True)
            st.markdown(
                "<div class='formula'>"
        ...
        with c3:
            st.markdown("<h4 style='color:#cbd5e1 !important;-webkit-text-fill-color:#cbd5e1 !important;font-size:1.05rem;font-weight:600;margin-bottom:.6rem;'>Signal 3: ML (RF)</h4>", unsafe_allow_html=True)
            st.markdown(
                "<div class='formula'>"
    ...
    with tab2:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("<h4 style='color:#cbd5e1 !important;-webkit-text-fill-color:#cbd5e1 !important;font-size:1.05rem;font-weight:600;margin-bottom:.6rem;'>UCB1 — Naive</h4>", unsafe_allow_html=True)
            st.code(
                "arms   = [momentum, mr, ml]\n"
        ...
        with c2:
            st.markdown("<h4 style='color:#cbd5e1 !important;-webkit-text-fill-color:#cbd5e1 !important;font-size:1.05rem;font-weight:600;margin-bottom:.6rem;'>UCB — Improved</h4>", unsafe_allow_html=True)
            st.code(
                "# 1. Signal Filtering\n"
                "if abs(signal) < 0.1:\n"
                "    signal = 0\n\n"
    ...
    with tab4:
        ...
        st.markdown("<h3 style='color:#e2e8f0 !important;-webkit-text-fill-color:#e2e8f0 !important;font-size:1.3rem;font-weight:700;margin-bottom:.8rem;border-bottom:1px solid #334155;padding-bottom:.4rem;'>Latest 10 Rows</h3>", unsafe_allow_html=True)
